...
### 方式零：GitHub Pages（纯静态演示，零服务器）

> **线上地址**：https://kevinkaslana093.github.io/gengxia/

游戏运行本身不需要服务端（引擎、素材、音效全在前端），所以可以整站导出为静态文件托管。
静态版包含 10 个预设游戏，**点开即玩**。

| 能力 | 静态版（Pages） | 完整版（本服务） |
| --- | --- | --- |
| 玩预设游戏、分享链接、挑战分数 `?c=` | ✅ | ✅ |
| 输入自己的梗生成新游戏 | ❌（需要模型密钥） | ✅ |
| 生成后的游戏可编辑 | ❌ | ✅ |

静态构建方式（`scripts` 之外的构建脚本见 `deploy/static/README.md`）：

```bash
STATIC_BASE=/<仓库名>/ python build-static.py   # 产出 static-build/
# 再把 static-build/ 推到 gh-pages 分支
```

> 关键坑位（都已在代码里处理，改动时别踩回去）：
> 1. **`<base href>` 必须指向站点根**（`/gengxia/`），否则子目录页面里 `/js/*` 会 404；
> 2. 路由要能容忍 **`.html` 后缀、尾斜杠、仓库名前缀**；
> 3. `pushState` 与分享链接必须带站点前缀，否则分享出去的地址 404；
> 4. 必须放 **`.nojekyll`**，否则 Pages 的 Jekyll 构建直接报 `Page build failed`。
